...
const Wm = ({ fill = C.t1, w = 100 }) => <svg viewBox="0 0 986.77 174.91" width={w} style={{ display: "block" }}>{WP.map((d, i) => <path key={i} d={d} fill={fill} />)}</svg>;
  ...
  return (
    <div>
      ...
      <div style={{ display: mobile ? "block" : "grid", gridTemplateColumns: "280px 1fr", gap: S.lg }}>
        {/* Controls */}
        <div style={{ display: "flex", flexDirection: "column", gap: S.sm, marginBottom: mobile ? S.md : 0 }}>
          <div style={{ ...cardS, padding: S.md + 4 }}>
            ...
            <div style={{ display: mobile ? "flex" : "block", gap: 6, overflowX: mobile ? "auto" : "visible", paddingBottom: mobile ? 4 : 0 }}>
              {TEAM.map((t, i) => (
