...
const SIG_DIR = join(ROOT, "public/sig");

mkdirSync(LOGO_DIR, { recursive: true });
mkdirSync(SIG_DIR, { recursive: true });

const VIEWBOX = "0 0 986.77 174.91";
const WP = [
...
function makePaths(fill, stroke, strokeWidth) {
  return WP.map(
    (d) =>
      `<path d="${d}" fill="${fill}"${stroke ? ` stroke="${stroke}" stroke-width="${strokeWidth}"` : ""}/>`
  ).join("\n  ");
}

function buildSvg(fill, stroke, strokeWidth, viewBox = VIEWBOX) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}">
  ${makePaths(fill, stroke, strokeWidth)}
</svg>`;
}
...
// Email signature logo (Black variant, 300px wide)
const sigPng = svgToPng(svgs[0].svg, 2);
